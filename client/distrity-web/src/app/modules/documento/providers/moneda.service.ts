...
export class MonedaService {
  ...
  search(term: string): Promise<Moneda[]> {
    term = term ? term : 'void';
  ...
  getBySigla(sigla: string): Promise<Moneda> {
  ...
  update(moneda: any): Promise<Boolean> {
    const url = `${this.url}/update/${moneda.id}`;
    return this.http
               .put(url, JSON.stringify(moneda), {headers: this.getHeaders()})
               .toPromise()
               .then(request => request.json() as Boolean)
               .catch(this.handleError);
  }

  create(moneda: any): Promise<Boolean> {
    const url = `${this.url}/add`;
    return this.http
               .post(url, JSON.stringify(moneda), {headers: this.getHeaders()})
               .toPromise()
               .then(request => request.json() as Boolean)
               .catch(this.handleError);
  }
